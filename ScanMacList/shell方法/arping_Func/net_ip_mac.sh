...
do
	...
	if [ "$Reply"x = "reply"x ];
        then
            ip1=$(arping $ip.$i -I $eth -c 1|grep "Unicast reply from"|awk '{print $4}')
            mac1=$(arping $ip.$i -I $eth -c 1|grep "Unicast reply from"|awk '{print $5}'|awk -F[ '{print $2}'|awk -F] '{print $1}')
